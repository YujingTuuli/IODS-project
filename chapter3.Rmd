...
This data set includes the information of 370 students' background, achievement, and alcohol consumption in two Portuguese schools.
There are 35 variables in the data, including student grades, demographic, social and school related features, as well as students' performance in Mathematics (mat) and Portuguese language (por).
...
cv$delta[1]
```
The prediction error rate is 24%, outperforming the model in Exercise Set 3, which had about 26% error. According to the result of 10 fold cross-validation, the model has an average error rate of 23.5%, a bit lower than the results from training model.
